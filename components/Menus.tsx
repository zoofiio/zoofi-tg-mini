...
export function Menus() {
    ...
    return <div className="flex items-center gap-2.5 w-full">
        {
            menus.map(m =>
                <div
                    onClick={() => m !== current && !m.disable && useMenusStore.setState({ current: m })}
                    className={cn("rounded-lg w-[3.75rem] h-[3.25rem] flex flex-col items-center justify-center gap-1 bg-white/95 text-black/50 cursor-pointer", { 'text-primary': m == current, 'cursor-not-allowed': m.disable, })}>
